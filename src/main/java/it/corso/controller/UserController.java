...
public class UserController {
	...
	@DELETE
	@Path("/delete/{email}")
	public Response delUser(@Valid @PathParam("email") String email) {
	    try {
	...
	public Response delUserById(@Valid @PathParam("id") int id) {
	    try {
	...
	private UtenteLoginResponseDTO issueToken(String email) {
		...
		Date end = java.sql.Timestamp.valueOf(LocalDateTime.now().plusMinutes(100L));
